...
function main() {
  ...
  // -------------------------------------------------------------------------
  // Demo working state: the reviewer's first login must never be an empty
  // shell (spec §7.1) — lists, drafts, and a search history that shows both
  // a corrected search and a blocked one.
  // -------------------------------------------------------------------------
  const listsSpec: Array<{ id: string; name: string; pick: (c: CompanyRow) => boolean; max: number; hoursAgo: number }> = [
    { id: "ls_demo1", name: "Austin SaaS targets", pick: (c) => c.city === "Austin" && c.industryId !== "legal-services", max: 8, hoursAgo: 3 },
    { id: "ls_demo2", name: "Healthcare shortlist", pick: (c) => c.industryId === "healthcare-software", max: 8, hoursAgo: 26 },
    { id: "ls_demo3", name: "FinTech follow-ups", pick: (c) => c.industryId === "fintech-software", max: 6, hoursAgo: 74 },
  ];

  const insertList = conn.prepare(
    "INSERT INTO lists (id, userId, name, createdAt, updatedAt) VALUES (?, 'user_demo', ?, datetime('now', ?), datetime('now', ?))",
  );
  const insertItem = conn.prepare(
    "INSERT INTO list_items (id, listId, companyId, addedAt) VALUES (?, ?, ?, datetime('now', ?))",
  );
  let itemCount = 0;
  for (const spec of listsSpec) {
    const offset = `-${spec.hoursAgo} hours`;
    insertList.run(spec.id, spec.name, offset, offset);
    for (const company of companies.filter(spec.pick).slice(0, spec.max)) {
      insertItem.run(`li_demo${++itemCount}`, spec.id, company.id, offset);
    }
  }

  // 4 prefilled drafts from the Austin list — generated by the same
  // deterministic templates the compose screen uses.
  const canonicalById = new Map(
    (taxonomyJson as { industries: CanonicalIndustry[] }).industries.map((i) => [i.id, i]),
  );
  const contactForCompany = conn.prepare(
    "SELECT id, name, title, email FROM contacts WHERE companyId = ? LIMIT 1",
  );
  const insertDraft = conn.prepare(
    `INSERT INTO drafts (id, userId, companyId, contactId, subject, body, contextPoints, status, createdAt)
     VALUES (?, 'user_demo', ?, ?, ?, ?, ?, ?, datetime('now', ?))`,
  );
  const austinCompanies = companies
    .filter(listsSpec[0].pick)
    .slice(0, 4);
  austinCompanies.forEach((company, index) => {
    const contact = contactForCompany.get(company.id) as
      | { id: string; name: string; title: string; email: string }
      | undefined;
    const canonical = canonicalById.get(company.industryId) ?? null;
    const points = generateContextPoints(company, canonical, index % 3);
    const email = generateEmail(company, contact ?? null, canonical, points, index % 3);
    insertDraft.run(
      `dr_demo${index + 1}`,
      company.id,
      contact?.id ?? null,
  ...
  // 6 search-log entries — the dashboard's confidence histogram needs a
  // history, including one search the resolver refused to guess on.
  const insertLog = conn.prepare(
    `INSERT INTO search_logs (id, userId, query, resolvedIndustryId, confidence, band, resultCount, createdAt)
     VALUES (?, 'user_demo', ?, ?, ?, ?, ?, datetime('now', ?))`,
  );
  const logs: Array<[string, string | null, number, string, number, string]> = [
    ["computr software", "software-development", 95, "high", 6, "-2 hours"],
    ["Verical Saas – Landscaping Software", "vertical-saas-other", 95, "high", 18, "-5 hours"],
    ["healthcare software", "healthcare-software", 100, "high", 46, "-27 hours"],
    ["lawnkare softwear", null, 39, "low", 0, "-30 hours"],
    ["fintech", "fintech-software", 99, "high", 45, "-3 days"],
    ["security software", "cybersecurity-software", 95, "high", 25, "-6 days"],
  ];
  logs.forEach(([query, industryId, confidence, band, results, offset], index) => {
    insertLog.run(`sl_demo${index + 1}`, query, industryId, confidence, band, results, offset);
  });
